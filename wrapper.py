...
DEFAULT_GENERATIONS = 500  #: the default number of generations for which to run a simulation
...
class GameDynamicsWrapper(object):
    ...
    def simulate(self, num_gens=DEFAULT_GENERATIONS, graph=True, burn=0, return_labeled=True, start_state=None, class_end=False):
        ...
        # print("hpo", game.DEFAULT_PARAMS)
        # for _ in range(500):
        # met randomly picked state:
        # dyn = self.dynamics_cls(payoff_matrix=game.pm, player_frequencies=game.player_frequencies,**self.dynamics_kwargs)
        ...
        results, payoffs, true_states, strategies, pay = dyn.simulate(num_gens=num_gens, debug_state=start_state)
        # print(len(results), len(payoffs))
        # # print(payoffs)
        # # print(strat, fit)
        # # print(results[0])
        # # print(results[1])
        ...
        #     # # print("before", payoffs[playerIdx])
        #     payoffs[playerIdx] = numpy.delete(player, (0), axis=0)
        #     # # print("after", payoffs[playerIdx])
        ...
        if dyn.stochastic:
            ...
            if class_end:  # Only classify the final generation
                ...
                classi = numpy.zeros((len(strategies[0]), 3))
                # # print(classi[100, 0])
                # true_states =

                # true_states = numpy.vstack((true_states, zeros))
                for x, state in enumerate(zip(*strategies)):
                    if x > 0:
                        # # print(x, true_states[x-1])
                        classi[x, 0] = true_states[x-1]
                        # # print("before", state)
        ...
        if graph:
            # # print(strategies)
            # setupGraph(graph, game, dyn, burn, num_gens, results, payoffs, true_states)
            setupGraph(graph, game, dyn, burn, num_gens, strategies, pay, true_states, results, payoffs)
        else:
            if return_labeled:
...
class VariedGame(object):
    ...
    def __init__(self, game_cls, dynamics_cls, game_kwargs=None, dynamics_kwargs=None):
        ...
        # print("kwargs", self.game_cls, self.dynamics_cls, self.game_kwargs, self. dynamics_kwargs)
...
#     x = GameDynamicsWrapper('rejection_game_small.RGSmall', 'moran.Moran',  None, None)
